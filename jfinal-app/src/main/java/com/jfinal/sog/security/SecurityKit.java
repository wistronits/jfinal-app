...
package com.jfinal.sog.security;
...
import com.jfinal.sog.kit.servlet.RequestKit;
...
public class SecurityKit {
    ...
    public static <T extends Model> void logout(HttpServletRequest req, HttpServletResponse response) {
        ...
        if (cookie_user != null) {
            ...
            if (user != null) {
                CacheKit.remove(LOGIN_CACHE_SESSION, LOGIN_CACHE_SESSION + user.getLong(FnConst.TABLE_PK_COLUMN));
            }
        }
        //清除session
    ...
    private static <T extends Model> void setLoginMember(HttpServletRequest request, HttpServletResponse response,
        ...
        request.getSession().setAttribute(LOGIN_MEMBER_ID, user.getLong(FnConst.TABLE_PK_COLUMN));
    ...
    private static <T extends Model> String getLoginKey(T user, String ip, String user_agent) {
        return encrypt(String.valueOf(user.getLong(FnConst.TABLE_PK_COLUMN)) + '|'
